...
public class AssetsTreeListViewAdapter<T> extends ParentTreeListViewAdapter<T> {

    public AssetsTreeListViewAdapter(ListView mTree, Context context,
    ...
    public View getConvertView(final Node node, final int position, View convertView,
        ...
        if (node.isLeaf() && node.getRoleLevel() == 5) {//如果是最后一层,并且是房间层
            ...
            viewHolder.mTvStatus.setText(node.getData());
            viewHolder.labelName.setText(node.getName());
        } else {
            if (node.getRoleLevel() == 1 || node.getRoleLevel() == 3) {//园区和楼栋需要展示房间数量
                viewHolder.labelName.setText(node.getName() + " (" + node.getAllCount() + ")");
            } else {
                viewHolder.labelName.setText(node.getName());
            }
        ...
        viewHolder.mLlParent.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!node.isLeaf() || node.getRoleLevel() != 5) {//该层不是房间
